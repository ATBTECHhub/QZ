...
const CreateTest = () => {
  ...
  // const addTestId = useTestStore((state) => state.addTestId);
   ...
   const { testId } = useTestStore((state) => ({ testId: state.testId }));
  ...
  //   }
  // }, [testId]);
  // const onSubmit = async (payload, actions, isCreatingTest = false) => {
  //   try {
  //     let res;
  //     if (testId) {
  ...
  const onSubmit = async (payload, actions, isCreatingTest = false) => {
    try {
      ...
      if (!testId) {
        // Save Test - First Submission
        res = await Axios.post(
          Request.createTest,
          payload
        );
        if (res.data.savedTest._id) {
          setTestId(res.data.savedTest._id);
          toast.success("Test saved successfully!");

          if (isCreatingTest) {
            // Redirect for question addition if "Create Test" was clicked
            navigate(
              `/instructor-dashboard/create-test/${res.data.savedTest._id}/questions`
            );
          }
        } else {
          toast.error("Failed to save test. Try again.");
        }
      } else if (isCreatingTest) {
        // If test is already saved, go to questions page
        navigate(`/instructor-dashboard/create-test/${testId}/questions`);
      }
    } catch (error) {
